...
| Export | Rôle |
|---|---|
| `<PaymentModal open onClose target onDone>` | 🔒 La **fenêtre de paiement intégrée** (0.5.0, artboards Hub-Paiement-*) — remplace le Checkout Stripe embarqué, pour l'onboarding, l'activation d'un outil sans abonnement vivant et les packs. `target: PaymentTarget` = `{ tool }` · `{ tools: string[] }` · `{ pack }`. À l'ouverture : Stripe.js (paresseux, `stripePublishableKey`), **trois champs carte séparés** (numéro, expiration, code) dans des `.ds-input` du DS, style lu dans les jetons (police `--font-mono`, taille et couleurs du champ), Link non proposé ; récap lu dans le catalogue et `preview-subscription-change` (pack : prix du catalogue). **Au clic sur « Payer »** (qui vaut acceptation des CGV) : `create-payment { tools \| tool \| pack, cguAcceptees: true, cguVersion: 'abonnement-v2' \| 'pack-v2', facturation: { pays, nom?, entreprise?, ligne1?, codePostal?, ville? } }` → `stripe.confirmCardPayment(clientSecret, { payment_method: { card, billing_details } })` → 3D Secure si demandée (`BankConfirmOverlay`, fenêtre Stripe par-dessus) → attente des droits posés par le webhook (8 s au plus) → `onDone(outcome: SubscriptionChangeOutcome)`, que le hub rend avec `SubscriptionResultScreen`. Refus de carte : message sous son champ, le clic suivant réessaie **le même paiement**. `payment_processing` : on attend les droits (20 s), jamais de nouveau paiement. Refus du serveur (`already_subscribed`, `not_published`, `rate_limited`, `cgu_required`, `invalid_input`, `stripe_error`…) : message au-dessus de « Payer », qui reste le geste pour réessayer. Abonnement vivant : `mode: 'added'` (ajout au prorata par le serveur, 3D Secure de la carte enregistrée si demandée). Bureau : Modal à `--container-wide`, deux colonnes, hauteur fixe ; mobile (≤ 64 rem) : plein écran, « Payer » dans le pied fixe. `PaymentModalProps`. |
| `<PaymentView open onClose recap phase form onFormChange errors? banner? countries renderCard cardFocus? onPay payDisabled? layout? inline?>` | La VUE de la fenêtre, pilotée par props (démo, états des artboards). `phase: PaymentViewPhase` = `preparing` (squelettes à la place finale des champs carte, « Payer » inactif) · `ready` · `sending` (« Paiement en cours… », champs figés) · `bank` · `activating`. `renderCard(field, placeholder)` rend le contenu d'un champ carte. Sous « Payer » : la mention de consentement mot pour mot (`fr.paiement.form.consent`, lien « CGV » vers `CGV_URL`), puis « Paiement sécurisé par Stripe ». |
| Logique pure (testée) : `paymentTools`, `validateBilling(form)`, `paymentBody(target, facturation)`, `paymentRecap(target, catalog, preview)`, `paymentOutcome(…)`, `countryOptions()`, `EMPTY_BILLING`, `CGV_URL`, `CGU_VERSIONS` | `validateBilling` : nom sur la carte et pays obligatoires, adresse facultative (case décochée ou champs vides = pas d'adresse, un champ rempli = les trois attendus), entreprise envoyée avec la case. `paymentBody` écrit le corps exact de `create-payment`. `countryOptions` : codes ISO 2, noms du navigateur en français. Types `PaymentTarget`, `BillingForm`, `Facturation`, `FieldErrors`, `CardField`, `CreatePaymentData`, `PaymentRecap`, `PaymentRecapLine`. |
| `useCheckoutActivation()` → `{ state, clear }` · `CHECKOUT_PARAM` · `PAYMENT_PARAM` · `CHECKOUT_TOOLS_PARAM` · `CHECKOUT_TOOL_PARAM` · `CHECKOUT_PACK_PARAM` | Le retour d'un paiement : `?paiement=1&tools=a,b` (`create-payment`, 0.5.0) ou `?checkout=<session_id>&tools=a,b` (ancien checkout), `&tool=` / `&pack=` selon l'achat : sonde `tool_entitlements` chaque seconde, 20 s au plus — un droit `subscription` actif **pour chaque** outil de `tools` (ou `tool`), un droit `pack` de l'outil du pack si `pack`, sinon un droit `subscription` / `pack` écrit depuis l'arrivée sur la page — `pending` · `active` · `late` (`idle` sans paramètre). Invalide abonnement et droits à l'activation ; `clear()` retire les paramètres. `AppLayout` affiche « Activation en cours… » pendant `pending`. `CheckoutActivationState`. |
...
Le premier abonnement (et tout pack) passe par la `PaymentModal`. Tout changement d'un abonnement existant passe par `update-subscription`.

**Conteneurs câblés** (ce que le hub monte)

| Export | Rôle |
|---|---|
| `<ModifySubscriptionModal open onClose onDone>` | « Modifier mon abonnement » : outils publiés ou souscrits, état lu dans les droits, aperçu à chaque geste, paiement avec la 3D Secure. `onDone(outcome: SubscriptionChangeOutcome)` après un succès. `checkoutRequis` (pas d'abonnement vivant) → `PaymentModal` des outils ajoutés, `onDone` inchangé. `ModifySubscriptionModalProps`. |
| `<ActivateToolModal open onClose toolId onDone? fromClaude?>` | « Activer un outil » : aperçu `{ tool }`, `update-subscription { ajouter: [tool] }`. `checkoutRequis` → `PaymentModal` (depuis Claude : « Tu peux retourner dans Claude » après le paiement). Abonnement actif sans carte → « Ajouter une carte » (portail dans un nouvel onglet, aperçu relu au retour). `fromClaude` → « Tu peux retourner dans Claude » dans la modale au lieu d'`onDone`. `ActivateToolModalProps`. |
...
| `<FullScreenSheet title subtitle? onClose inline? footer?>` | Le plein écran des paiements sous 64 rem (fenêtre de paiement et modales d'abonnement), avec un pied fixe facultatif. |
...
## Retiré en 0.5.0 (fenêtre de paiement intégrée)

`CheckoutModal`, `CheckoutModalProps` (→ `PaymentModal`) · `useStartCheckout`, `checkoutTools`, `CheckoutTarget`, `CheckoutStart` (→ `create-payment` dans `PaymentModal`, `paymentTools`, `PaymentTarget`) · les textes `fr.parametres.abonnement.checkout`.
