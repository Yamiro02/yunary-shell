...
export const fr = {
  ...
  layout: {
    ...
    settings: 'Paramètres',
    /* La carte compte (maquette Hub Dashboard, 25/09/2026) : « 2 outils · 14 €/mois », ou « Gratuit » sans abonnement actif. */
    planFree: 'Gratuit',
    planTools: (n: number, perMonth: string | null) => `${n} outil${n > 1 ? 's' : ''}${perMonth ? ` · ${perMonth}/mois` : ''}`,
  ...
  parametres: {
    ...
    /* Ce qui reste de l'ancien onglet Abonnement (retiré en 0.4.0, remplacé par la page Facturation du hub) : les chaînes
       du checkout, de l'activation, de la résiliation et du bandeau de paiement, qui servent toujours. */
    abonnement: {
      ...
      /* Retour d'un paiement (`?checkout=` ou `?paiement=1`) : on sonde les droits jusqu'à 20 s. Jamais d'erreur rouge : le paiement a réussi. */
      activation: {
  ...
  /* Abonnement v2 (maquette « Yunary Hub Dashboard », 25/09/2026) : modifier, activer, réactiver, retour de paiement,
     résiliation programmée. Les noms d'outils, prix et quotas viennent de la base ; les dates sont formatées à l'affichage. */
  paiement: {
    perMonth: (price: string) => `${price}/mois`,
    priceLine: (price: string, quota: string) => `${price}/mois · ${quota}`,
    card: {
      label: (brand: string, last4: string) => `${brand} · •••• ${last4}`,
      expires: (mmYY: string) => `Expire ${mmYY} · paiement chiffré, géré par Stripe`,
      change: 'Changer',
      missingTitle: 'Ajoute une carte pour continuer',
      missingBody: "Aucune carte n'est enregistrée sur ton abonnement.",
      add: 'Ajouter une carte',
    },
    bank: {
      title: 'Confirme auprès de ta banque',
      body: (amount: string) => `Ta banque demande une validation pour ce paiement de ${amount}. Ouvre ton appli bancaire, ou saisis le code reçu par SMS.`,
      waiting: 'La fenêtre de ta banque est ouverte',
      reassurance: "Rien n'est débité sans ta validation.",
      cancel: 'Annuler le paiement',
      cta: 'En attente de ta banque…',
    },
    declined: {
      title: 'Ta carte a été refusée',
      body: "Rien n'a été débité et tes outils n'ont pas changé. Change de carte, ou demande à ta banque d'autoriser le paiement puis réessaie.",
    },
    ...
    saving: 'Enregistrement…',
    cancel: 'Annuler',
    todayLabel: "À payer aujourd'hui",
    nothingToday: "Rien à payer aujourd'hui",
    nextLabel: 'Ensuite',
    nextFrom: (date: string) => `à partir du ${date}`,
    nextFromShort: (date: string) => `dès le ${date}`,
    nextCharge: (date: string) => `Prochain prélèvement le ${date}`,
    prorata: (tool: string, from: string, to: string) => `${tool} du ${from} au ${to}`,
    modify: {
      title: 'Modifier mon abonnement',
      subtitle: 'Coche ce que tu gardes ou ajoutes, décoche ce que tu retires. Un retrait prend effet à la fin de la période déjà payée.',
      subtitleMobile: 'Ajout actif dès le paiement · retrait en fin de période',
      recap: 'Récapitulatif',
      adds: 'Tu ajoutes',
      removes: 'Tu retires',
      reactivates: 'Tu réactives',
      addedLine: (tool: string, price: string) => `${tool} (${price}/mois)`,
      removedLine: (tool: string, date: string) => `${tool}, actif jusqu'au ${date}, puis résilié`,
      noChange: "Aucun changement pour l'instant. Ajoute ou retire un outil au-dessus.",
      renewsOn: (date: string) => `Renouvelé le ${date}`,
      removalNote: (date: string, tool: string) => `Résiliation prévue le ${date}. En attendant, tu continues de profiter de ${tool} jusqu'à cette date.`,
      endingNote: (date: string) => `Actif jusqu'au ${date}, puis résilié`,
      reactivatedNote: (date: string) => `Réactivé, renouvelé le ${date}`,
      active: 'Actif',
      ending: 'Résiliation prévue',
    },
    activate: {
      title: 'Activer',
      done: (tool: string) => `${tool} est actif`,
      doneClaude: "Tu peux retourner dans Claude : l'outil y est disponible tout de suite.",
      close: 'Fermer',
    },
    reactivate: {
      title: 'Réactiver',
      body: (date: string) => `La résiliation prévue le ${date} est annulée. L'outil continue comme avant, sans coupure.`,
      alreadyPaid: 'La période en cours est déjà payée',
      cta: 'Réactiver',
    },
    result: {
      addedOne: 'est actif',
      addedMany: (n: number) => `Tes ${n} outils sont actifs`,
      addedLeadOne: "Disponible tout de suite. Lance-le depuis Claude quand tu veux.",
      addedLeadMany: 'Disponibles tout de suite. Lance-les depuis Claude quand tu veux.',
      removedTitle: 'Retrait programmé',
      removedLead: (tool: string, date: string) => `${tool} reste actif jusqu'au ${date}. Ensuite, plus rien n'est prélevé pour cet outil.`,
      failedTitle: "Le paiement n'est pas passé",
      failedLead: (amount: string) => `Ta banque a refusé les ${amount}. Rien n'a été débité, tes outils n'ont pas changé.`,
      paidToday: "Payé aujourd'hui",
      nextFrom: (date: string) => `Ensuite dès le ${date}`,
      badgeActive: 'Actif',
      badgeEnds: (date: string) => `Se termine le ${date}`,
      badgeFailed: 'Non ajouté',
      back: 'Retour à mes outils',
      retry: 'Réessayer avec une autre carte',
      invoicesBefore: 'Ta facture est dans ',
      invoicesLink: 'Facturation',
    ...
    cancellation: {
      ...
      body: (n: number) => (n > 1 ? `Tes ${n} outils restent actifs jusque-là. Plus rien ne sera prélevé ensuite.` : 'Ton outil reste actif jusque-là. Plus rien ne sera prélevé ensuite.'),
      keep: 'Garder mes outils',
    },
    /* La fenêtre de paiement intégrée (0.5.0, artboards Hub-Paiement-*). Noms et montants viennent de la base puis de l'Edge. */
    form: {
      titleOne: (tool: string) => `S'abonner à ${tool}`,
      titleMany: (n: number) => `S'abonner à ${n} outils`,
      titlePack: (pack: string) => `Acheter ${pack}`,
      subtitle: 'Facturé chaque mois, résiliable en un clic.',
      subtitlePack: 'Paiement unique, sans abonnement.',
      perMonth: (price: string) => `${price} / mois`,
      thenMonthly: (day: number) => `Puis le ${day === 1 ? '1er' : day} de chaque mois`,
      head: 'Paiement',
      cardTitle: 'Carte bancaire',
      preparing: 'On prépare le paiement sécurisé, quelques secondes…',
      ...
      cvcPlaceholder: '3 chiffres au dos',
      cvcPlaceholderShort: '3 chiffres',
      country: 'Pays',
      addAddress: 'Ajouter une adresse de facturation',
      addAddressHelp: 'Facultatif · pour une facture au nom de ton entreprise.',
      ...
      /* Mot pour mot (Julien, 27/09/2026) ; le back enregistre la même phrase sous `cguVersion`. « CGV » est le lien. */
      consent: {
        abonnement: { before: 'En payant, tu acceptes les ', link: 'CGV', after: ' et tu demandes que ton abonnement démarre tout de suite : tu renonces donc à ton droit de rétractation de 14 jours.' },
        pack: { before: 'En payant, tu acceptes les ', link: 'CGV', after: ' et tu demandes que ton pack soit disponible tout de suite : tu renonces donc à ton droit de rétractation de 14 jours.' },
      },
      errors: {
        name: 'Indique le nom sur la carte.',
        country: 'Choisis ton pays.',
        line1: "Indique l'adresse.",
        postalCode: 'Indique le code postal.',
        city: 'Indique la ville.',
        numberIncomplete: 'Le numéro de carte est incomplet.',
        numberInvalid: "Ce numéro de carte n'est pas valide.",
        expiryIncomplete: "La date d'expiration est incomplète.",
        expiryPast: 'Cette carte a expiré.',
        expiryInvalid: "Cette date d'expiration n'est pas valide.",
        cvcIncomplete: 'Le code de sécurité est incomplet.',
        cvcInvalid: "Le code de sécurité n'est pas bon.",
        declined: 'Ta carte a été refusée. Essaie une autre carte ou appelle ta banque.',
        failed: "Le paiement n'est pas passé. Réessaie dans un instant.",
        processing: "Ton paiement est en cours de validation. Tes outils s'activent dès qu'il est confirmé, inutile de payer à nouveau.",
        cguRequired: 'Accepte les CGV pour payer.',
        invalidInput: 'Vérifie tes informations de facturation, puis réessaie.',
        stripe: "Le paiement n'a pas pu être préparé. Réessaie dans un instant.",
        prepare: "Impossible de préparer le paiement. Réessaie.",
      },
    },
  },
