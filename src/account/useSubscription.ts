 ...
 * ⚠️ GRANT SELECT colonne-limité (user_id, plan, status, current_period_end, cancel_at_period_end) :
...
export function useSubscription() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SubscriptionInfo | null> => {
      ...
      if (!data) return null;
      return { plan: data.plan, status: data.status, currentPeriodEnd: data.current_period_end, cancelAtPeriodEnd: data.cancel_at_period_end };
    },
  });
}
